...
f=open('Merged_count_9bpdel_knock.txt','w')
for filename in allsample:
    ...
    for c in read:
        c=c.split('\t')
        if 'deletion <50 nt' in c and c[6].split(',')[1]=='9':
            ndel+=1
    f.write('\t'.join(['knock-knock',sample,pacbio[sample],str(ndel),str(len(read)),str(ndel/len(read)*100)])+'\n')
f.close()
